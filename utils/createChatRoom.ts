...
import { addDoc, getFirestore, setDoc, doc, getDoc, collection, arrayUnion, updateDoc } from "firebase/firestore";
...
export default async function createChatRoom(currentUserID: string, otherUserID: string, navigation: any){
    try{
        let foundRoom = false;
        ...
        if(!foundRoom){
            ...
            await updateDoc(doc(db, 'Users', `${currentUserID}`), {PrivateChatRooms: arrayUnion(
                {chatRoomId: createdRoom.id, otherUser: otherUserName}
            )})
            await updateDoc(doc(db, 'Users', `${otherUserID}`), {PrivateChatRooms: arrayUnion(
                {chatRoomId: createdRoom.id, otherUser: currentUserName}
            )})
